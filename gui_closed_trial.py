...
# Defining trial dictionaries so they can be logged and converted to a json file to be sent over ZMQ
class Trial:
    def __init__(self, trial_number, reward_port, volume_state, correct_trial, volume_changes, pokes, reward_state):
        self.trial_number = trial_number # Any integer
        self.reward_port = reward_port # Any integer between 1-8
        self.volume_state = volume_state # States: low, normal, high
        self.volume_changes = volume_changes # Any integer
        self.pokes = pokes
        self.correct_trial = correct_trial # True or Flase
        self.reward_state = reward_state # True or False
        self.timestamp = datetime.now()


    def to_dict(self):
        """Convert the Trial object to a dictionary."""
        return {
            'trial_number': self.trial_number,
            'reward_port': self.reward_port,
            'volume_state': self.volume_state,
            'volume_changes': self.volume_changes,
            'pokes': self.pokes,
            'correct_trial': self.correct_trial,
            'reward_state': self.reward_state,
            'timestamp': self.timestamp.strftime('%H:%M:%S')
        }

    def to_json(self):
        """Convert the Trial object to a JSON string."""

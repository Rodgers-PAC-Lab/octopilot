    ...
    def update(self):
        ...
        n_pokes = int(
            np.sum(
                [
                    len(pokes_on_port)
                    for pokes_on_port in self.dispatcher.history_of_pokes.values()
                ]
            )
        )
        ...
        n_correct_trials = int(
            np.sum(
                [
                    len(rewards_on_port)
                    for rewards_on_port in self.dispatcher.history_of_rewarded_correct_pokes.values()
                ]
            )
        )

        # Number of incorrect trials (number of rewarded incorrect pokes)
        n_incorrect_trials = int(
            np.sum(
                [
                    len(rewards_on_port)
                    for rewards_on_port in self.dispatcher.history_of_rewarded_incorrect_pokes.values()
                ]
            )
        )

        # The total number of unique ports poked per trial
        rcp_times_ntrials = int(
            np.sum(self.dispatcher.history_of_ports_poked_per_trial)
        )

        # Time since session start
        if self.dispatcher.session_start_time is not None:
            time_from_session_start_sec = (
                datetime.datetime.now() - self.dispatcher.session_start_time
            ).total_seconds()
        else:
        ...
        if n_pokes > 0:
            last_poke = np.max(
                np.concatenate(list(self.dispatcher.history_of_pokes.values()))
            )
        ...
        self.correct_count.setText(f"N correct trials: {n_correct_trials}")

        # Update fraction correct label
        if fraction_correct is not None:
            self.fraction_correct.setText(f"fraction correct: {fraction_correct:.2f}")
        else:
            self.fraction_correct.setText(f"fraction correct: NA")
        ...
        if time_from_session_start_sec is not None:
            ...
            self.time_label.setText("elapsed time: NA")
        ...
        if time_from_last_poke is not None:
            ...
            self.poke_time_label.setText("time since poke: NA")
...
class PokePlotWidget(QWidget):
    ...
    def update_time_bar(self):
        ...
        if self.dispatcher.session_start_time is not None:
            ...
            approx_time_in_session = (
                current_time - self.dispatcher.session_start_time
            ).total_seconds()
            ...
            self.line_of_current_time.setData(
                x=[approx_time_in_session, approx_time_in_session],
                y=[-1, 9],
    ...
    def update_plot(self):
        ...
        * Extracts pokes from
        ...
        pairs = (
            (self.dispatcher.history_of_pokes, self.plot_handle_unrewarded_pokes),
            (
                self.dispatcher.history_of_rewarded_correct_pokes,
                self.plot_handle_rewarded_correct_pokes,
            ),
